...
- **2026-09-03 three types + cruise speed on the card (both repos, on disk).** Discord (CorporalSimmons, a 1947 save): (1) **Curtiss C-46 Commando** `c46` — the catalogue's earliest freighter was the 1959 An-12, so cargo was unplayable in a 1950 world. Surplus (no era new-build premium), eis 1942 / oop 1945, 6.8 t, 300 km/h, 1,900 km, 3,600 ft, $1.4M / $3.5K, maint $7K, banded 832w. Priced off the freighter guards, not sentiment: the scrap-value floor (cheapest new-build $/t ÷ 8 = $0.16M/t) and the <10× spread cap put a hard floor near $1.26M. Own family `curtiss_c46` (no pax sibling; added to the consistency test's `ALLOWED_FREIGHT_ONLY` beside the two Antonovs). Deliberately NOT in `CLASSIC_FREIGHTERS`: that guard compares maint/tonne against modern new-builds, which a 6.8 t piston twin cannot meet without a 1.4%-of-hull weekly bill. (2) **Canadair North Star** `northstar` — eis 1947 / oop 1950, 55 seats, 520 km/h, 6,200 km, $2.5M / $6.4K, fuel 162, maint $14K. A DC-4 on Merlins: strictly faster/longer/bigger than the DC-4, paid for in $/seat, fuel per seat (2.95 vs 2.68) and its OWN family `canadair_northstar` at $22K/wk (the orphaned engine line), so a DC-4 operator adding one takes a second family base cost. (3) **Lockheed L-1049** `l1049` — the pre-G Super Constellation, eis 1951 / oop 1954, 88 seats, 480 km/h, range held to 4,300 km so the Stratocruiser keeps the long-haul niche; $4.6M / $11.9K, maint $21K (plain R-3350s, under the G's turbo-compound $24K). Deadweight report: all three land in "viable alternatives" (peaks 89–96% of the winner), none dominant. **Cruise speed is now a listed spec** — `cruiseSpeedKmh(type)` on the market card pill row, a sortable `speed` column in the comparison table, the checkout header line, and a Cruise column in the generated aircraft guide (both generators). `era-availability-test` 1950 count 6 → 8. Suites: TW 109/109, HW 155/156 (sandbox-only adsense EPERM). **Deploy note: HW `apps/headwinds-web/public/` still needs `node tools/headwinds-public.mjs` on a real machine** — the bridge can't delete, so the rebranded aircraft pages there are stale until then.

- **2026-09-16 the 1960s short-haul jets (both repos, on disk).** Dave: a mid-60s era world had no 727 until 1972 and no DC-9 until 1975. (1) **Boeing 727-100** `b727100` — eis 1964 / oop 1972, 131 seats, 4,100 km, 5,800 ft, $8M / $19.5K, fuel 510 (3.89/seat vs the -200's 3.14), crew 1.70 (three-crew), maint $50K, `boeing_727`. (2) **Douglas DC-9-10** `dc910` — eis 1965 / oop 1968, 90 seats, 2,300 km, 5,000 ft, $5.5M / $13K, fuel 360, crew 1.30, maint $27K. (3) **Douglas DC-9-30** `dc930` — eis 1967 / oop 1982, 115 seats, 2,800 km, 5,400 ft, $7.5M / $19K, fuel 395 (3.43/seat: between the 737-200's 3.17 and the One-Eleven's 4.03), crew 1.35, maint $38K. Both DC-9s in `mcd_md80`. All banded 832w, all above the ≤1969 $42K/seat floor. (4) **`b727200` re-dated: eis 1972 → 1967 and renamed "Boeing 727-200"** — the entry was the -200 Advanced (ANA, 1972) but the stretch entered service with Northeast in Dec 1967; numbers unchanged, so the June-block vintage/wifi/APU rules are untouched. Deadweight report (classic): all three land in "viable alternatives" beside their contemporaries (727-100 59%, DC-9-30 58% ≥ One-Eleven 55%, DC-9-10 51%). TW `generate-aircraft-pages.mjs` header/footer templates were missing the Ko-fi Support link (added by hand in f1496c4) — fixed in the template so a regen no longer strips it. Suites: HW 185/188 (adsense + worlds-pages sandbox EPERM, fuel-impact-ui is the in-flight fuel-visibility work), TW 140/140, golden PARITY OK. TW devlog entry + sitemap bump.
...
- **2026-09-02 balance sweep on the new prices (`tools/playbot-era.mjs`, HW engine, NWR + crew, no rivals, 2 years):** classic/737-800 −24% year one, +18% year two. 1950/C-47 **+370% / +1138%** (14 tails, ~$2.3M/wk revenue); 1950/CV-240 +612% / +1533%; 1950/DC-4 +702%; 1978/737-200 +259% / +1034%; 1978/F27 +168%. **The new-build premium did not move these** — a $12K/wk CV-240 lease against ~$250K/wk of route profit is noise. The era's fare index (1950: 1.55 × NWR 0.95 ≈ 1.47) on a monopoly pool that the bot fills to 95% load is the lever: per-flight revenue is ~2× per-flight cost on a JFK–IAD C-47. Real 1950 fares WERE that high in today's money; real 1950 load factors were ~60%, not 95%. Candidate levers, Dave to pick: γ_yield (fare index 1.55 → ~1.2), an era load-factor/demand haircut, or accept — in a full world ten humans split that same pool. Also: the crew-shortage parking order prefers idle then newest tails. (The `eraOnly` hiding of the 23 propliners from classic lasted one night — see the next bullet.)
- **2026-09-02 vintage rule (both repos):** Dave: every aircraft belongs on the classic market. `eraOnly` is gone. Instead `vintageDeliveredAgeWeeks(type)` in aircraft.js: a banded type whose line closed ≥ `VINTAGE_AFTER_YEARS` (50) ago is delivered in classic at `VINTAGE_AGE_FLOOR` (20y) + a year per year past 50, capped at `VINTAGE_AGE_CAP` (30y → 5.5× maintenance); `isVintage(type)` = that exceeds the published band (32 types: DC-3, C-47, the 1950s block, the 1960s jets, L-188, CV-580, An-12, 747-100; NOT the Il-18/HS748/707-320/An-24 at 47–48y, nor the Concorde which has no band). Vintage = buy-only in every classic world (HW `leaseDenial` checks it ahead of the NWR gate; TW got a new `leaseDenial`). `valueRemaining` shifts the depreciation line so a frame delivered at the floor still books at catalogue on arrival and runs down over `VINTAGE_NAV_LIFE_YEARS` (10), capped at 1.0 for legacy frames younger than the band. Classic now refuses only `withdrawnYear` types (Comet 1). Era worlds untouched (`eraDeliveredAgeWeeks(t, 2026)` still equals the published band).

- **2026-09-24 lessor age rule (both repos):** Dave: era lessors only carry a type 10+ years after EIS. `LESSOR_MIN_AGE_YEARS` (10) + `lessorFirstYear(type)` in aircraft.js; `surplus` types (C-46, C-47, DC-4) exempt, leasable from EIS. HW: in `lessorSupplies`' era branch, so `leaseDenial` (NWR worlds) and the Marketplace agree, message "Lessors take the X from YYYY"; classic untouched (golden PARITY OK). TW: new era branch in `leaseDenial` (code `lessor_age`, gates LEASE + lease ORDER) + Marketplace `leaseLockReason`. 1950 lease book: DC-3, C-46, C-47, DC-4 only; CV-240 from 1958, DC-6B/L-1049 1961, 707 1968, 727 1974, 747-200 1981. Existing leases/orders unaffected. Tests: `tools/era-lease-age-test.mjs` (both, failing on HEAD); era-availability (HW) and era-pricing (TW, CV-240 lease moved to 1958) updated.
