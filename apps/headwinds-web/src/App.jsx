        ...
        <label>Starting capital ($)
          ...
          <span className="muted">
            {Number.isInteger(startYear)
              ? <>Airlines founded in {startYear} start with exactly <strong>{fmtMoney(Number(startingCapital))}</strong>; later joiners scale with the era from there. For reference, the modern $10.0M is worth about {fmtMoney(eraSeedCapital(10_000_000, startYear))} in {startYear}.</>
              : <>{fmtMoney(Number(startingCapital))} per airline · default $10.0M</>}
          </span>
        </label>
...
const MOVE_LABELS = {
  ...
  BUILD_GROUND_STATION: (p) => `Started a ground handling station at ${p.code ?? p.airportCode ?? '?'}`,
  UPGRADE_GROUND_STATION: (p) => `Upgrading the ground station at ${p.code ?? p.airportCode ?? '?'}`,
  CLOSE_GROUND_STATION: (p) => `Closed the ground station at ${p.code ?? p.airportCode ?? '?'}`,
  SIGN_CATERING_CONTRACT: (p) => `Signed a ${p.years ?? '?'}-year catering contract`,
  BREAK_CATERING_CONTRACT: () => 'Broke a catering contract early',
