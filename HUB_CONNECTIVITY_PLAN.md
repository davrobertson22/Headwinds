...
**Rollout — flag ON for the alpha worlds and every future world (Dave, 2026-09-07; Headwinds server, on disk).** `tickConfig.rivalItineraries` is now read as *on unless explicitly `false`*: `tickService` stamps `rivalItineraries: tc.rivalItineraries !== false` on every airline blob each tick, `worldConfig` publishes the same, `worldService` stores the key only when a creator turns it off and bakes the default at join, and the admin route `POST /worlds/:id/rival-itineraries {enabled}` stores `false` / deletes the key. Existing worlds therefore pick the feature up on their next tick with no migration. Tailwinds: `freshState` and `reconcileState` already default it on.
