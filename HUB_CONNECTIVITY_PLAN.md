...
**Package-wide gating + beta-world golden — BUILT 2026-09-07 (both repos, on disk).** Dave's review question ("have we made the right decisions?") surfaced that only the rival one-stops were behind the flag: the 1a spill/choke rewrite, through-fares and the time-scaled penalty on own connections, partner feed seating and the multi-tail fix all reached every world unconditionally — a +68% cash swing (974M → 1,633M on the probe) landing on the beta worlds mid-game. Now `state.rivalItineraries` is ONE switch for the whole package (`network.js hubPackageOn`):
- `rivalIndexFor(state)` returns the frozen `RIVALS_OFF` sentinel when off (never null — "the caller passed nothing" is the modern rules with no rivals, `undefined`), an empty index when on with nobody else in the world, else the built index. `isLegacy(idx)` / `rivalsOn(idx)` are the two questions every call site asks.
- `computeMarketShare(market, offers, { legacy })` dispatches to `_computeMarketShareLegacy` — the function VERBATIM from 818c2ff, the last commit before the package's first engine change. Own-metal and partner offers on a legacy world use the flat tier penalty, sum-of-legs fares and the pre-package entry shapes; `weeklyTick` skips feed seating, the per-tail feed share and the partner report fields; pairShare and the RouteDetail/RoutePlanner share panels pass the same `legacy` so previews agree with the tick on a beta world.
- **`tools/golden-master/beta-world.mjs`** — the second golden master: the probe's two-hub network with static rivals (`multiplayer: true`, as a Headwinds world) and the flag off, 30 weeks, full-state hash. Its baseline was captured by running the same file against the engine at 818c2ff (extracted with `git archive`); the current engine matches it byte-for-byte. Before gating it mismatched (cash 622M → 845M, own-metal pax 2,555 → 70,532 on the flag-OFF world). `tools/beta-world-parity-check.mjs` runs it inside the normal suite. Re-baseline only for a change MEANT to reach the betas, and say so.
- The first golden master (flag unset in HW `freshState`) re-baselined: that scenario is now a legacy world by definition, projection identical to the dollar. TW `freshState` now sets `rivalItineraries: true` (it was only set at START_GAME; previews built on a fresh state fell to legacy while the reconciled provider state was modern — preview-honesty-test caught it).
- Probe with static-rival parity in hand: flag OFF 951M (the 23M gap to 974M is the AI capacity change moving the AI carriers, which Headwinds does not have), flag ON 1,217M → **1,234M with the penalty scale below**.

**Connection penalty ×2 — BUILT 2026-09-07 (`CONNECTION_PENALTY_SCALE = 2` inside `connectionPenaltyFor`, modern rule only).** Dave: "shouldn't there be a debuff for through fares as most pax prefer a direct flight?" There was — the tier/partnership penalties — but in log-odds a T2 base of 0.32 let a one-stop take ~42% of a pair at equal fare; airline QSI practice is a fifth to a third. Two findings from measuring it:
- **Why it has no effect on the dense probe** (×1 → ×3 moves 2-yr cash 1%): connections there are seat-bound, not preference-bound. Week 52 at ×1 vs ×3: itineraries WIN 69,002 → 37,017 pax/wk (−46%), but only 21,377 → 17,096 get a SEAT on the legs (direct pax already fill 74% of seats; each connecting pax uses two legs), and the hub's external pool + partner feed expand into every seat the penalty frees (same `capScale`). Revenue from seated connections actually rose 7.5M → 8.0M (the time-scaled penalty kills the worst-detour, lowest-yield routings first). The probe now prints `connections: own-metal won X → seated Y` so this is visible every run. **Implication for tuning hub power: the lever is seat headroom (the 18% protected fraction, the 95% ops buffer), not the penalty and not the fare index.**
- **Where it does bite — `tools/thin-network-probe.mjs`** (`@not-a-test`, HW only: Raleigh T1 hub, eight 1,100–1,900 pax/wk spokes at 14×/wk × 180 seats so every flight has room, two static Headwinds-shaped hub rivals over ORD and ATL reaching Raleigh and every spoke). The stock AI flies big cities only, so this is the human-rival case. Package OFF: LF 45%, 430k pax/yr. ON at scale ×1: rival one-stops on 6/8 pairs take **52.6%** per pair (two one-stops out-selling the nonstop), 285k pax; ×2: **40.9%** (≈20% each — QSI territory), 314k pax; ×3: 30.2%, 346k pax. ×2 chosen. `itinerary-quality-test` now pins the scale and the equal-fare share (30–37% for a T2 one-stop).
- THROUGH_FARE_INDEX stays 1.0: measured 1.1 → 1,178M (−3%), 1.15 → ~883M, 1.2 → 668M — a cliff where the through-fare crosses the fare choke and the spoke legs empty; 1.1 buys nothing and parks one notch from the shelf.
- Suites: HW 167/168 (sandbox adsense) incl. both goldens PARITY OK; TW 120/120. Fixtures for package behaviour (`partner-feed-seats-test`) now set `rivalItineraries: true` explicitly; `rival-itinerary-test` covers the sentinel semantics.
