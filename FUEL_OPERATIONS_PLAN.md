...
**Date:** 2026-09-19
**Status:** ALL SIX PHASES BUILT — 1 + 2 on 2026-09-19, 3, 4, 5 and 6 on 2026-09-20 (Headwinds; Tailwinds port pending). Still open: 3c age creep (open question 3), §7.6 regional drift, §8.3 farm events, and the remaining §7.3 chip surfaces (Routes/CargoRoutesList, RouteFinder, tag/cargo planners, map layer).
Deviations from plan, Phase 6: **the crack walk's σ is 0.035 applied directly, not 0.03 × 2.5.** The plan borrowed the jet walk's shock shape, but that walk reverts at θ = 0.06; at the crack walk's θ = 0.03 the same shock gives a stationary sd of ~0.18 — wider than the whole [0.95, 1.45] band — so the index sat on both clamps and the band's asymmetry (0.17 below the 1.12 mean, 0.33 above) dragged the realised long-run mean to **1.171**. The test asserts the realised mean over 20k steps, which is what caught it. `REFINING_COST` is 0.04, not 0.05, to land the plan's own stated ~7% at the long-run crack (0.05 gave 5.7%). Added a size gate the plan did not specify, `REFINERY_MIN_WEEKLY_BILL` = $150M/wk: at the long-run crack a refinery clears its opex only above ~$75M/wk, so without a gate the desk sold a guaranteed loss to a mid-table airline — the plan's own arithmetic says it is an endgame purchase, and the gate now enforces that. Capacity is measured in litres off `lastReport` (price multiplier and base price divided out) and fixed at purchase, so the share falls as the airline grows; the same estimator sizes it and measures need, so the ratio is self-consistent. The outage is a per-airline roll inside ADVANCE_WEEK **gated on owning a running refinery** — not a world event template — so a save without one draws no random number and both goldens hold. `fuelPrice.crack` is written only on a save that owns a refinery and is seeded at purchase, so nobody else's blob gains a key and no world's fuel history is rewritten; multiplayer replays a shared `worldCrackIndex` on its own `crack:` salt. Hedges are credited against `hedgeableShare` (1 − refinery share) so an owner is not paid twice for the same litres. Rivals cannot see a refinery — unlike a farm it is a balance-sheet decision, not territory. **Bonus fix:** `decomposeWeek` now prefers a history entry's own recorded `fuelMultiplier`, which has been stored since the Phase 2 scoreboard but never read — without it every past week of a refinery owner's fuel card would have understated the base bill. Verified economics: five seeded 10-year runs on a $250M/wk airline return 3.9 / 7.1 / 10.0 / 20.1 / 5.7% a year on the $2.5B, and **every** seed has losing years (worst −$176M). Tests: `tools/fuel-refinery-test.mjs` (18), `tools/fuel-refinery-ui-test.mjs` (9).
Deviations from plan, Phase 5: `state.fuelFarms` is an object keyed by airport (`{ code, level, builtAbsWeek, capex }`), not an array; capex tiers map to the catalogue's three airport tiers (regional $8M / major $30M / mega $80M for a stake, ×4 for a farm). The discount reaches the sims as a per-station discount map on the station module (`setFuelStationDiscounts`, set from `farmDiscountsOf(state)` at the same four knob sites), so `effectiveStationBasis` = table basis × (1 − discount) and tankering decisions use the airline's own price. Throughput fees ride the existing `DividendCredit` table with a new `kind` column (`'dividend'` | `'farm_fee'`, migration `20260920000000_farm_fee_credits`) rather than a new table; the tick issues `farm_fee` credits from each landed airline's `fuelByStation` at stations another airline's level-2 farm holds (half for alliance-mates via `loadAllianceMap`), and the next tick hands them in as `action.incomingFarmFees` → `tickInput.farmFeeIncome` → `report.totalFarmFeeIncome`, booked inside `totalRevenue`/`cashDelta` so tax, cash and the P&L bridge reconcile (bridge row `farmFees`; history `farmFees`). One owner per airport is enforced in the reducer against the injected rival views (`competitors[].fuelFarms`, published by `toHumanCompetitor`); the server guard checks only the airport code, so two airlines building in the same instant is a known small race. No events (§8.3). Owned-farm cap `max(1, floor(fleet/25))`. UI: `FuelFarmControls` on the Stations card (new Fuel farm column) and Airport Detail (the plan's "Competition airport view"); P&L income row for fees. Golden masters: `PARITY OK` on both (all farm fields written only when farms exist). **Deploy needs `npx prisma generate` and the migration applied.** Tests: `tools/fuel-farm-test.mjs` (17), `tools/fuel-farm-ui-test.mjs` (6).
Deviations from plan, Phase 4: the station factor is applied INSIDE the three route sims (`routeFuelStations` in `data/fuelStations.js`, gated by a module knob `setFuelStationsEnabled` set from `state.fuelOpsV` the way `setNwrYieldChoke` is — reducer wrapper, `weeklyTick`, the client provider, the server rival builder), so every preview and the tick agree with no call-site changes; the plan's `routeFuelFactor` helper became `resolveFuelForWeek` (price × burn, Phase 3) + `routeFuelStations` (station × tankering, per route). Stated overrides apply AFTER the population-weighted normalisation so a stated 1.10 is 1.10 (final table's weighted mean 1.001; JFK 0.92, LHR 0.99, DXB 0.86, NAN 1.20). `START_GAME` stamps `fuelOpsV: 2` on every new solo game and `createWorld` writes `tickConfig.fuelOpsV: 2`; the join path drops the seeded key and takes the world's, and the tick reads it live, so an admin flip of `tickConfig.fuelOpsV` reaches every airline. **Golden master re-baselined** (`--update`, stated): the JFK–LAX scenario now starts on station pricing and its final cash moved −$45.32M → −$43.86M, the only projection field that changed; the beta-world golden was NOT re-baselined — its fixture drops the stamped key as a pre-feature world's airline would, and prints PARITY. Per-route report fields (`fuelStationFactor`, `fuelStationBasis`, `fuelByStation`, `tankering`) and `report.fuelByStation` are written only while the knob is on. UI surfaces done: `FuelBasisChip`, Airports (sortable Fuel column), AirportDetail, AirportSelect/OriginPicker, HubManagement, RouteDetail (chips, Fuel/wk stat, tankering Auto/Off), RoutePlanner pair header, Finance Stations card. Still to do from §7.3: Routes/CargoRoutesList (Dave editing Routes.jsx), RouteFinder, Tag/Cargo planners, the optional map layer; Competition airport view lands with Phase 5. Tests: `tools/fuel-stations-test.mjs` (20), `tools/fuel-stations-ui-test.mjs` (6).
