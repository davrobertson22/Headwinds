...
// with it) and then on the few spec fields the tick can see. The client never
// mutates a state in place — a poll adopts a fresh blob, an overlay spreads a
// new object, a local action goes through the reducer — so a stale hit is
// impossible. Nothing in the tick reads the probe's aircraft: the memo is
// exact, not approximate, and planner-memo-test is what says so.
...
export function projectConnectingFeed(state, spec) {
  ...
  // type — 75 times on a long sector — and the seasonal strip twelve more times
  // per row, all against the same state. Measured on a 279-route solo save that
  // was ~3.5 s of blocking work on every keystroke in the Route Planner, most
  // of it here (TheCookiesGuy, Discord, 14 Sep 2026; the same shape as the
  // 11–12 Sep lag reports on Headwinds). Remember the answer per (state, lane,
  // frequency, fare, calendar) and hand it back.
