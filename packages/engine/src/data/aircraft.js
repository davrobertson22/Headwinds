...
//
// Optional per-world rule set (World.tickConfig.newWorldRestrictions → the
// engine flag state.newWorldRestrictions). NOTHING below has any effect unless
// that flag is set, so classic worlds and Tailwinds solo are untouched.
//
// Two rules, aimed at two different failure modes observed in the live world
// "Scarce Assets" (an airline leased 196 A380s across two decisions two minutes
// apart — an 8.8x fleet expansion funded entirely by 12-week deposits):
//
//   1. Lessors carry single-deck, previous-generation aircraft only. Anything
//      bigger or newer must be bought — new, or used from another airline.
//   2. Your lease order book is capped relative to the fleet you already
//      operate, so growth happens in waves with deliveries and P&L in between
//      rather than in one click.
//
// Rule 1 is deliberately NOT just a year test. A pure `eis <= cutoff` rule
// blocks the A380 (2007) but leaves every other double-decker open, and they
// are all older: the 747-400 is 71% of the A380's seats for 30% of the rent,
// and the 747-300 is cheaper still. A year rule alone would have made the
// strategy ~6x cheaper rather than stopping it. Hence the explicit
// `doubleDeck` flag, which is carried on the FREIGHTER 747s too — those sit in
// category 'Freighter', so a category-based test would miss the 747-400F and
// hand the same strategy straight to cargo.

/** Entry-into-service cutoff: lessors stock airframes certified up to this year. */
export const LESSOR_EIS_CUTOFF = 2000;

// Post-cutoff types the lease market genuinely does carry in volume. Without
// these the regional lease market collapses to the CRJ200 and ERJ145, which
// reads as broken to anyone who knows the segment: the E-Jets and CRJ700+ are
// the most-leased regional aircraft in the world, and the current-build
// turboprops are lessor staples.
export const LESSOR_ALLOW = new Set([
  'erj170', 'e175', 'e190', 'e195',      // E-Jet E1 generation
  'crj700', 'crj900', 'crj1000',
  'a318',                                 // A320-family straggler (2003)
  'b737900er',                            // 737NG straggler (2007)
  'atr42', 'atr72',                       // current-build ATRs
  'c208b', 'dhc6',                        // utility turboprop workhorses
]);

// Pre-cutoff types with no real secondary lease market — prestige or one-off
// airframes a lessor could never remarket, so none of them sit on a lease book.
export const LESSOR_BLOCK = new Set([
  'concorde',
  'an124', 'an225',                       // outsize Soviet freighters
]);
...
export function aircraftOrderable(type, calYear = null) {
  // Classic worlds: the catalogue is timeless — every type is on the 2026
...
// block; the 1950s propliners added for era worlds share the 16y band but burn
...
// published bands: classic worlds only, never era worlds (their calendar is
 ...
 *   - classic world (calYear null): the published table, lifted by the vintage
...
export function eraDeliveredAgeWeeks(type, calYear = null) {
  ...
  // A line that closed long before 2026 but carries no band is a classic-world
  // conceit (the Concorde: still sold factory-fresh in 2026, prestige over
  // realism — and its maintenance is balanced on that). An era world in the
  // period has no such licence — a 1990 Concorde is a used 1979 frame — so it
  // ages on the oldest band UNTIL 2026, where the modern catalogue's own rule
  // resumes: an era world at 2026 is the classic market, by invariant (see
  // era-availability-test), and classic itself is untouched.
  const band = published > 0 ? published : (type.oop < 2005 && calYear < 2026 ? 832 : 0);
  if (band > 0) {
    const span = Math.max(1, 2026 - type.oop);
    return Math.max(0, Math.min(832, Math.round(band * (calYear - type.oop) / span)));
  }
...
export function leasableTypes() {
  return AIRCRAFT_TYPES.filter(lessorSupplies);
}

// ── Rule 2: lease order book ─────────────────────────────────────────────────
// Cap on leased aircraft ON ORDER at once, as a share of the fleet already in
// service. A rate limit, not a level cap: it never stops an airline growing, it
// forces the growth to happen in waves. Real carriers run order books that are
// a fraction of the operating fleet, not a multiple of it.
//
// The floor lets a startup place a meaningful first order; 25% overtakes it at
// 20 aircraft, which is where the proportional rule starts to bind.
export const LEASE_ORDER_BOOK_PCT = 0.25;
export const LEASE_ORDER_BOOK_MIN = 5;

/** How many leased frames this airline may have on order, given its active fleet. */
export function leaseOrderBookCap(activeFleetCount) {
  const n = Math.max(0, Math.floor(Number(activeFleetCount) || 0));
  return Math.max(LEASE_ORDER_BOOK_MIN, Math.floor(n * LEASE_ORDER_BOOK_PCT));
}
